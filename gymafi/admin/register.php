                        ...
                        <div class="col-xl-6">
                            <div class="card spur-card">
                                <div class="card-header">
                                    ...
                                    <div class="spur-card-title"> Register a new user here: </div>
                                </div>
                                <div class="card-body">
                                ...
                                if(isset($_POST['clientsubmit'])) {
                                    ...
                                    $read = "SELECT * FROM users WHERE first_name = '$firstname' AND last_name = '$lastname' AND postcode = '$postcode' ";
                                    $result = $conn->query($read);
                                    ...
                                    if($numrows > 0) {
                                        echo '<p class="text-center text-danger">Account already exists!</p>';
                                    } else {
                                        $clientinsert = "INSERT INTO users (first_name, last_name, username, email, pword, postcode, house_number, phone_number, is_admin, ass_coach_id) VALUES ('$firstname', '$lastname', '$username', '$email', '$pword', '$postcode', '$housenumber', '$phonenumber', $userrole, $id)";
                                        ...
                                        if(!$result) {
                                            ...
                                            $imgquery = "INSERT INTO profile_img (img_id) VALUES ($lastid)";
                                            $getimg = $conn->query($imgquery);
                                            if(!$getimg) {
                                                echo $conn->error;
                                            } else {
                                                // echo "<meta http-equiv='refresh' content='0'>";
                                                echo '<p class="text-center text-success">Account created!</p>';
                                            }

                                            if($userrole==1) {
                                                $coachinsert = "INSERT INTO coaches (user_id) VALUES ($lastid)";
                                                $coachresult = $conn->query($coachinsert);
                                                if(!$coachresult) {
                                                    $conn->error;
                                                } else {
                                                    echo '<p class="text-center text-success">...and coach added.</p>';
                                                }
                                            }
                                    ...
                                    ?>
                                    <form class="new-client" method="POST" action="../admin/register.php">
                                        ...
                                        <div class="form-group">
                                            <input name="pword" type="password" pattern=".{5,12}" class="form-control" placeholder="Password" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <input name="housenumber" type="number" class="form-control" placeholder="House Number" required>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <div class="row">
                                                <div class="col-sm-10">
                                                    <div class="custom-control custom-radio">
                                                        <input value="0" type="radio" id="customRadio1" name="userrole" class="custom-control-input">
                                                        <label class="custom-control-label" for="customRadio1">Client</label>
                                                    </div>
                                                    <div class="custom-control custom-radio">
                                                        <input value="1" type="radio" id="customRadio2" name="userrole" class="custom-control-input">
                                                        <label class="custom-control-label" for="customRadio2">Coach</label>
                                                    </div>
                                                </div>
                                            </div>
                                        </fieldset>
                                        <button name="clientsubmit" type="submit" class="btn btn-dark mt-3">Register</button>
                                    </form>
                                </div>
                            </div>
                        </div>
