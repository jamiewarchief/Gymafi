                                ...
                                <div class="card-body">
                                    <form action="messages.php" method="POST">
                                        <div class="form-group">
                                            <label for="coachselect"><strong>Choose a coach</strong></label>
                                            <select name="coachselect" class="form-control bg-dark text-white" id="coachselect">
                                            <?php

                                                $read = "SELECT * FROM users WHERE is_admin = 1";
                                                $result = $conn->query($read);
                                                if(!$result) {
                                                    echo $conn->error;
                                                }
                                                ...
                                                while($row=$result->fetch_assoc()) {
                                                    $get_coach_first_name = $row['first_name'];
                                                    $get_coach_last_name = $row['last_name'];
                                                    $get_coach_id = $row['id'];
                                                    echo '<option value='.$get_coach_id.'>'.$get_coach_first_name.' '.$get_coach_last_name.'</option>';
                                                }
                                            ?>
                                            </select>
                                        </div>
    ...
    <?php
        ...
        if(isset($_POST['sendmessage'])) {
            ...
            $newmessagequery = "INSERT INTO messages (subject, message, sender_id, recipient_id)
                                VALUES ('$messagesubject', '$messagetosend', $id, $coachselect)";
            ...
            $mailboxquery = "INSERT INTO mailboxes (user_id, mailbox, message_id)
                            VALUES ($id, 'out', $lastid), ($coachselect, 'in', $lastid)";
